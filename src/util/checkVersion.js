...
module.exports = async (dirPath, serverUrl, currentVersion) => {
  ...
  try {
    var res = await axios.get(serverUrl + "/version/integrador");
    if (res.data.version != currentVersion) {
      log.writeToLog("New Version Detected");
      var pathToNewIntegrador = path.resolve(dirPath, "Integrador-Novo.exe");
      var writer = fs.createWriteStream(pathToNewIntegrador);
      writer.on("finish", () => {
      ...
      writer.on("error", (err) => {
        log.writeToLog("Error downloading new integrador version");
        log.writeToLog(err);
      });

      var response = await axios({
        url: serverUrl + "/version/integrador/" + res.data.version + ".exe",
        method: "GET",
        responseType: "stream",
      });
