module.exports = async (firebird, log) => {
  var tablesDone = await firebird.queryOne(
    `SELECT chave,valor FROM INTEGRADOR_CONFIG WHERE chave = ?`,
    ["initial_sync_tables_done"],
    ["chave", "valor"]
  );
  ...
  tabelas.sort((a, b) => {
    ...
    if (isAPrioritario && isBPrioritario) {
      if (
        tabelasPrioritarias.indexOf(nomeA) > tabelasPrioritarias.indexOf(nomeB)
      ) {
        return 1;
      }
      if (
        tabelasPrioritarias.indexOf(nomeB) > tabelasPrioritarias.indexOf(nomeA)
      ) {
        return -1;
      }
    }
  ...
  await firebird.execute(
    "UPDATE OR INSERT INTO parametros (IDCHAVE,VALOR) VALUES ('integrador', 'S')"
  );
  await firebird.execute(
    "select rdb$set_context('USER_SESSION', 'INTEGRADOR_PRIORIDADE', 3) from rdb$database",
    []
  );
  ...
  for (var key in tabelas) {
    ...
    if (tablesDone.indexOf(tabela.nome) == -1) {
      ...
      if (tabelasPrioritarias.indexOf(tabela.nome) != -1) {
        ...
        await firebird.execute(
          "select rdb$set_context('USER_SESSION', 'INTEGRADOR_PRIORIDADE', 1) from rdb$database",
          []
        );
      }

      await firebird.execute(
        `UPDATE ${tabela.nome} SET SINC_UUID = SINC_UUID`,
        []
      );

      tablesDone.push(tabela.nome);
      await firebird.execute(
        "UPDATE OR INSERT INTO INTEGRADOR_CONFIG (chave,valor) VALUES (?,?)",
        ["initial_sync_tables_done", JSON.stringify(tablesDone)]
      );
    }
  }
  ...
  await firebird.execute(
    "select rdb$set_context('USER_SESSION', 'INTEGRADOR_PRIORIDADE', null) from rdb$database",
    []
  );
};
